...
# Mauricio
library(ClusterR)
wine_scaled_new <- wine_scaled %>% dplyr::select(-color)

gmm = GMM(wine_scaled_new, 4, 
          dist_mode = "eucl_dist", 
...
pr = predict(gmm, newdata = wine_scaled_new)

wine_scaled$cluster <- NA
wine_scaled$cluster <- pr


ggplot(wine_scaled, 
       aes(fixed_acidity, residual_sugar, color = as.factor(cluster))) +
  geom_point()

gmm2 = GMM(wine_scaled_new, 3, 
          dist_mode = "eucl_dist", 
          seed_mode = "random_subset", 
          km_iter = 10, 
          em_iter = 5, 
